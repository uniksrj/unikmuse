...
<head>
    ...
    <title>{{ $categoryName }} - Latest Articles & Insights | Unik Muse</title>
    <meta name="description"
        content="Explore comprehensive {{ $categoryName }} content including guides, tutorials, news, and expert insights. {{ $posts->total() }}+ curated articles.">
    <meta name="keywords" content="{{ $categoryName }}, articles, guides, tutorials, news, insights">
    <meta property="og:title" content="{{ $categoryName }} - Expert Insights & Articles | Unik Muse">
    <meta property="og:description"
        content="Dive into our collection of {{ $posts->total() }}+ {{ $categoryName }} articles curated for enthusiasts and professionals.">
    <meta property="og:image" content="{{ asset('storage/' . ($posts->first()->image ?? 'assets/category-banner.jpg')) }}">
    ...
    <main class="container-unik py-8 md:py-12">
            ...
            <div class="category-gradient rounded-unik-xl text-white p-8 md:p-12 relative overflow-hidden">
                ...
                <div class="relative z-10 text-center">
                    <div class="inline-flex items-center gap-3 mb-6">
                        ...
                        <span class="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full text-sm font-medium">
                            <i class="fas fa-newspaper mr-2"></i> {{ $posts->total() }}+ Articles
                        </span>
                    </div>
                    ...
                    <div class="flex flex-wrap justify-center gap-4 mb-8">
                        ...
                        <div class="text-center">
                            <div class="text-2xl font-bold">
                                {{ $posts->where('created_at', '>=', now()->subDays(30))->count() }}
                            </div>
                        ...
                        <div class="text-center">
                            <div class="text-2xl font-bold">
                                @php
                                    $mostViewed = $posts->sortByDesc('views')->first();
                                @endphp
                                {{ $mostViewed->views ?? 0 }}
                            </div>
                    ...
                    <nav class="text-sm opacity-80" aria-label="Breadcrumb">
                        <ol class="flex justify-center items-center space-x-2">
                            ...
                            <li><i class="fas fa-chevron-right text-xs"></i></li>
                            <li class="font-medium" aria-current="page">{{ $categoryName }}</li>
                        </ol>
                    </nav>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="md:flex">
                            <div class="md:w-1/2 relative">
                                ...
                                <div class="absolute top-4 left-4">
                                    <span
                                        class="bg-unik-primary text-white px-3 py-1 rounded-full text-xs font-bold">
                            ...
                            <div class="md:w-1/2 p-6 md:p-8">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 bg-unik-primary/10 rounded-full flex items-center justify-center">
                                            <i class="fas fa-user text-unik-primary"></i>
                                        </div>
                                        <div>
                                            <div class="font-medium text-sm">{{ $featuredPost->author_name ?? 'Admin' }}</div>
                ...
                <!-- Ad Banner -->
                <div class="bg-gradient-to-r from-unik-light to-white rounded-unik-lg p-6 border border-unik-border text-center">
                    <div class="inline-flex items-center gap-2 mb-3">
                        <i class="fas fa-ad text-unik-muted"></i>
                        <span class="text-sm text-unik-muted font-medium">Sponsored</span>
                    </div>
                    <p class="text-unik-dark mb-4">Elevate your {{ strtolower($categoryName) }} skills with our premium
                        resources</p>
                    <a href="#" class="inline-flex items-center text-unik-primary font-medium hover:text-unik-secondary">
                ...
                <section>
                    <div class="flex justify-between items-center mb-8">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <select class="bg-white border border-unik-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-unik-primary"
                            ...
                            @foreach($posts as $post)
                                <article
                                    ...
                                    <div class="relative overflow-hidden rounded-t-unik-lg">
                                        @php
                                            $imagepath = json_decode($post->file_path, true);
                                            $img = !empty($imagepath) 
                                                ? ($imagepath['original']['webp'] ?? $imagepath['original']['jpg'] ?? 'uploads/no_image.jpg')
                                    ...
                                    <div class="p-6">
                                        <div class="flex items-center gap-2 mb-3">
                                            ...
                                            <span class="text-unik-muted text-xs">
                                                ...
                                                {{ \Carbon\Carbon::parse($post->created_date)->diffForHumans() }}
                                            </span>
                                        </div>
                                        ...
                                        <div class="flex items-center justify-between pt-4 border-t border-unik-border">
                                            <div class="flex items-center gap-4 text-xs text-unik-muted">
                                                ...
                                                <span class="flex items-center gap-1">
                                                    ...
                                                    {{ round(str_word_count($post->description) / 200) }} min read
                                                </span>
                                            </div>
                                            <a href="{{ route('post.show', $post->id) }}"
                ...
                <!-- Newsletter Section -->
                <section class="bg-gradient-to-r from-unik-primary to-unik-secondary rounded-unik-xl p-8 text-white text-center">
            ...
            <aside class="lg:col-span-1 space-y-6 sticky-sidebar">
                ...
                <div class="bg-white rounded-unik-lg shadow-unik-sm p-6 border border-unik-border">
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="font-bold text-unik-primary">{{ $posts->total() }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                ...
                <!-- Ad Box -->
                <div class="bg-gradient-to-br from-unik-secondary to-unik-primary rounded-unik-lg p-6 text-center text-white">
                    <div class="mb-4">
                        <i class="fas fa-gem text-3xl opacity-80"></i>
                    </div>
                    <h4 class="font-bold text-lg mb-2">Premium {{ $categoryName }} Toolkit</h4>
                    <p class="text-sm opacity-90 mb-4">
                        Access exclusive resources, templates, and tools for mastering {{ strtolower($categoryName) }}
                    </p>
                    <a href="#"
                        class="inline-block bg-white text-unik-primary px-6 py-3 rounded-lg font-bold hover:bg-gray-100 transition-colors">
                        Upgrade Now
                    </a>
                </div>
                    ...
                    <div class="space-y-2">
                        ...
                        @foreach($relatedCategories as $related)
                            <a href="{{ route('category.show', $related) }}"
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-lg bg-unik-primary/10 flex items-center justify-center group-hover:bg-unik-primary/20 transition-colors">
                                        <i class="fas fa-folder text-unik-primary text-sm"></i>
                                    </div>
                                    <span class="font-medium text-unik-dark">{{ ucwords(str_replace('-', ' ', $related)) }}</span>
                                </div>
                                <i class="fas fa-chevron-right text-unik-muted group-hover:text-unik-primary transition-colors"></i>
                            </a>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-white rounded-unik-lg shadow-unik-sm p-6 border border-unik-border">
                    ...
                    <div class="space-y-4">
                        @foreach($posts->take(3) as $trending)
                            <a href="{{ route('post.show', $trending->id) }}"
                                ...
                                <div class="flex-shrink-0 w-10 h-10 rounded-lg bg-unik-primary/10 flex items-center justify-center group-hover:bg-unik-primary/20 transition-colors">
                                    <span class="font-bold text-unik-primary">{{ $loop->iteration }}</span>
                                </div>
                                <div>
                                    <h4 class="font-medium text-unik-dark group-hover:text-unik-primary transition-colors text-sm">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        ...
                                        <span class="text-xs text-unik-muted">{{ $trending->created_at->diffForHumans() }}</span>
                                    </div>
                                </div>
                            </a>
                        @endforeach
        ...
        <!-- Author Spotlight -->
        @if($topAuthor = $posts->groupBy('author_id')->sortByDesc(function($posts) {
            ...
            <section class="mt-12 bg-white rounded-unik-xl p-8 border border-unik-border">
                ...
                <div class="flex flex-col md:flex-row items-center gap-8">
                    <div class="md:w-1/4">
                        <div class="w-48 h-48 mx-auto rounded-full bg-gradient-to-r from-unik-primary to-unik-secondary p-1">
                    ...
                    <div class="md:w-3/4">
                        <h3 class="text-2xl font-bold text-unik-dark mb-3">{{ $topAuthor->author_name ?? 'Expert Author' }}
                        </h3>
                        <p class="text-unik-muted mb-6 leading-relaxed">
                            With over {{ $posts->where('author_id', $topAuthor->author_id)->count() }} articles in
                            {{ $categoryName }}, our featured author brings deep expertise and practical insights.
                            Their work has been read {{ $posts->where('author_id', $topAuthor->author_id)->sum('views') ?? '50,000+' }}
                            times.
                        </p>
