    ...
    <div class="container-unik py-8 md:py-12">
        ...
        <section class="mb-16">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
                @foreach ($featuredPosts as $post)
                    <article class="bg-white rounded-unik-lg shadow-unik-md border border-unik-border hover:shadow-unik-lg transition-all duration-300 hover:-translate-y-1" 
                        ...
                        <div class="p-6">
                            <span class="inline-block bg-unik-primary text-white px-3 py-1 rounded-full text-sm font-medium mb-3">
                                {{ $post['category'] }}
                            </span>
                            <h3 class="text-lg md:text-xl font-semibold text-unik-dark mb-3 line-clamp-2" itemprop="headline">
                                {{ $post['title'] }}
                            </h3>
                            <p class="text-unik-muted mb-4 text-sm md:text-base line-clamp-3" itemprop="description">
                                {{ Str::limit($post['excerpt'], 120) }}
                            </p>
                            <a href="#" class="block px-5 py-2 bg-unik-primary items-center  color-unik-primary font-medium hover:color-unik-secondary transition-colors text-sm"
                               itemprop="mainEntityOfPage">
                                Read More <i class="fas fa-arrow-right ml-2"></i>
                            </a>
                        </div>
                        <meta itemprop="datePublished" content="{{ Carbon::parse($post['published_at'])->toIso8601String() }}">
                        <meta itemprop="dateModified" content="{{ Carbon::parse($post['updated_at'])->toIso8601String() }}">
        ...
        <section class="mb-16">
            ...
            <div class="flex flex-wrap justify-center gap-3 md:gap-4">
                <button class="px-5 py-2.5 rounded-full border border-unik-border hover:border-unik-primary hover:text-unik-primary transition-colors {{ !$category ? 'bg-unik-primary text-white border-unik-primary hover:text-white hover:bg-unik-primary/90' : 'bg-white text-unik-dark' }}"
                        onclick="window.location='{{ route('blog.index') }}'">
                    All Categories
                </button>
                @foreach ($categoriesList as $catName => $count)
                    <button class="px-5 py-2.5 rounded-full border border-unik-border hover:border-unik-primary hover:text-unik-primary transition-colors {{ $category == $catName ? 'bg-unik-primary text-white border-unik-primary hover:text-white hover:bg-unik-primary/90' : 'bg-white text-unik-dark' }}"
                ...
                <section>
                            ...
                            <article class="bg-white rounded-unik-lg shadow-unik-md border border-unik-border hover:shadow-unik-lg transition-all duration-300 hover:-translate-y-1" 
                                ...
                                <div class="p-6">
                                    <div class="flex justify-between items-center mb-4">
                                        <span class="bg-unik-primary/10 text-unik-primary px-3 py-1 rounded-full text-xs font-medium">
                                            {{ $post['category'] }}
                                        </span>
                                    ...
                                    <p class="text-unik-muted mb-6 text-sm md:text-base line-clamp-3" itemprop="description">
                                    ...
                                    <div class="flex justify-between items-center pt-4 border-t border-unik-border">
                                        ...
                                        <a href="#" class="inline-flex items-center text-unik-primary font-medium hover:text-unik-secondary transition-colors text-sm">
                                            Read More <i class="fas fa-arrow-right ml-2"></i>
                                        </a>
                                    </div>
                                </div>
                    ...
                    <div class="flex justify-center items-center gap-2 mt-12">
                        {{-- @if ($posts->onFirstPage())
                            <span class="px-4 py-2 rounded-unik-md border border-unik-border text-unik-muted cursor-not-allowed">◀ Previous</span>
                        @else
                            <a href="{{ $posts->previousPageUrl() }}" class="px-4 py-2 rounded-unik-md border border-unik-border text-unik-primary hover:border-unik-primary hover:bg-unik-primary/5 transition-colors">◀ Previous</a>
                        @endif

                        @foreach (range(1, $posts->lastPage()) as $page)
                            @if ($page == $posts->currentPage())
                                <span class="px-4 py-2 rounded-unik-md bg-unik-primary text-white border border-unik-primary">{{ $page }}</span>
                            @else
                                <a href="{{ $posts->url($page) }}" class="px-4 py-2 rounded-unik-md border border-unik-border text-unik-primary hover:border-unik-primary hover:bg-unik-primary/5 transition-colors">{{ $page }}</a>
                            @endif
                        @endforeach

                        @if ($posts->hasMorePages())
                            <a href="{{ $posts->nextPageUrl() }}" class="px-4 py-2 rounded-unik-md border border-unik-border text-unik-primary hover:border-unik-primary hover:bg-unik-primary/5 transition-colors">Next ▶</a>
                        @else
                            <span class="px-4 py-2 rounded-unik-md border border-unik-border text-unik-muted cursor-not-allowed">Next ▶</span>
                        @endif --}}
                    </div>
                </section>
            </main>
                        ...
                        @foreach ($categoriesList as $catName => $count)
                            <li>
                                <a href="{{ route('blog.index', ['category' => $catName]) }}" 
                                   class="flex px-5 py-2 bg-unik-primary justify-between items-center p-3 rounded-unik-md hover:bg-[#5f6397] text-unik-primary transition-colors group">
                                    <span class="font-medium">{{ $catName }}</span>
                                    <span class="bg-unik-light text-unik-dark text-xs px-2 py-1 rounded-full">{{ $count }}</span>
                                </a>
                            </li>
                        @endforeach
                    </ul>
                ...
                <div class="bg-white rounded-unik-lg shadow-unik-sm p-6 border border-unik-border">
                    ...
                    <div class="space-y-4">
                        @foreach ($popularPosts as $popular)
                            <div class="flex items-start gap-3 p-2 rounded-unik-md hover:bg-unik-primary/5 transition-colors group">
                                <img src="{{ asset('storage/' . $popular['thumbnail']['original']['webp']) }}" 
                                     alt="{{ $popular['title'] }}" 
                                     class="w-16 h-16 object-cover rounded-unik-md flex-shrink-0">
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-medium text-unik-dark text-sm mb-1 line-clamp-2 group-hover:text-unik-primary transition-colors">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            blogCards.forEach(card => {
                ...
                const datePublished = card.querySelector('meta[itemprop="datePublished"]')?.getAttribute('content');
                const dateModified = card.querySelector('meta[itemprop="dateModified"]')?.getAttribute('content');
